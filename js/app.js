...
var moreContentIgnition = false;

function moreContent(){
	if(moreContentIgnition === false){
		moreContentIgnition = true;
		document.getElementById('more').innerHTML = bacon;
	}else{
		moreContentIgnition = false;
...
var lessContentContainer = '';

var lessContentIgnition = false;

function lessContent(){
	if(lessContentIgnition === false){
		lessContentIgnition = true;
		lessContentContainer = document.getElementById('less').innerHTML;
		document.getElementById('less').innerHTML = "";
	}else{
		lessContentIgnition = false;
		document.getElementById('less').innerHTML = lessContentContainer;
	}
}
...
var menuItems = ['Apple Pie', 'Breakfast Burrito', 'Filet -O-Fish'];

function valueMenu(){
	if(menuDisplay === false){
		for(var i = 0; i < menuItems.length; i++){
			var node = document.createElement('li');
			var textnode = document.createTextNode(menuItems[i]);
